...
for (n in 1:2) {
  ...
  cov_ch_all <- subset(swiss_cov, as_date(date) %in% seq(min(simu_alpha$date),max(simu_delta$date),1))
  seq_ch_variant <- subset(seq_ch, as_date(date) %in% seq(min(sim_voc$date),max(sim_voc$date),1))
  
  print(c(min(variant_data$date),max(variant_data$date))) #time_window
  ...
  print(paste0("variant proportion:  ", round(mean(variant_data$proportion)*100,2)))
  ...
  print(paste0("Variant sequenced (%):  ", round(sum(variant_ch$who_variants %in% variant)/sum(variant_ch$num_seq_variant)*100,2)))
  ...
  mean_surveillance_delay+qnorm(0.975) *(sd_surveillance_delay/sqrt(n_surveillance_delay))
  mean_surveillance_delay-qnorm(0.975) *(sd_surveillance_delay/sqrt(n_surveillance_delay))
