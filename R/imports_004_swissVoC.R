...
#imports_alpha <-read.csv("./data/old/liberalClusters-Alpha_10022022.csv")[,-1]
#imports_conservative_alpha <- read.csv("./data/old/conservativeClusters-Alpha_10022022.csv")[,-1]
...
proportion_plot_Re_t <- ggplot(data= sim)+
  ...
  geom_line(aes(x=date, y=T2/T_total, color=approach), size=2)+
  scale_color_manual(values= col_9[c(2,4)],name=variant_name, labels= c("liberal","conservative")) +
  theme_minimal()+
...
cases_day_plot_Re_t <-  ggplot(data= sim)+
  geom_line(data = cov_ch, aes(x = date, y = weigthed_cases),color=col_9[9], size=2)+
  geom_line(aes(x=date, y=T2, color=approach), size=1.5)+
  scale_color_manual(values= col_9[c(2,4)],name=variant_name, labels= c("liberal","conservative")) +
  geom_line(aes(x=date, y=T_total,group=approach), size=1.5)+
  theme_minimal()+
...
if(n %in% c(1)){
  ...
  proportion_plot_Re_t_alpha <- ggplot(data= sim)+
    geom_errorbar(data= variant_data, aes(x = date, ymin=lower, ymax=upper), color= col_9[9], alpha=0.5,width=.1) +
    geom_point(data= variant_data,aes(x = date, y=proportion), color= col_9[9])+
    geom_line(aes(x=date, y=proportion, color=approach), size=2)+
...
plot_model_ch1 <- ggarrange(plot_model_ch_alpha1, plot_model_ch_delta1,
                           ncol = 2, nrow = 1)
ggsave(plot_model_ch1, filename = paste0("./data/figures/Figure21_import_variants_",format(Sys.time(), "%Y-%m-%d"), ".pdf"), height = 4, width = 6,  bg = "transparent")

ggsave(plot_model_ch1, filename = paste0("./data/figures/Figure21_import_variants_",format(Sys.time(), "%Y-%m-%d"), ".png"), height = 4, width = 6,  bg = "transparent")


plot_model_ch <- ggarrange(plot_model_ch_alpha, plot_model_ch_delta,
                           ncol = 2, nrow = 1)
ggsave(plot_model_ch, filename = paste0("./data/figures/Figure2_import_variants_",format(Sys.time(), "%Y-%m-%d"), ".pdf"), height = 8, width = 8,  bg = "transparent")

ggsave(plot_model_ch, filename = paste0("./data/figures/Figure2_import_variants_",format(Sys.time(), "%Y-%m-%d"), ".png"), height = 8, width = 8,  bg = "transparent")

#proportion_plot_abstract <- ggarrange(proportion_plot_Re_t_alpha, proportion_plot_Re_t_delta, labels = c("A","B"), ncol = 2, nrow = 1)
#ggsave(proportion_plot_abstract, filename = paste0("./data/figures/Figure_Abstract_proportionVoC_",format(Sys.time(), "%Y-%m-%d"), ".png"), height = 3, width = 8,  bg = "transparent")
