...
for (n in 1:2) {
  if(n==1){
    ...
    #variant_data <- alpha_ch[alpha_ch$date %in% seq(min(alpha_ch$date),min(alpha_ch$date[alpha_ch$proportion==max(alpha_ch$proportion)]),1),]
    variant_name <- "Alpha"
  ...
  if(n==2){
    ...
    #variant_data <- delta_ch[delta_ch$date %in% seq(min(delta_ch$date),min(delta_ch$date[delta_ch$proportion==max(delta_ch$proportion)]),1),]
    variant_name <- "Delta" 
...
plot_logistic_fit <- ggarrange(plot_alpha_logistic_fit, plot_delta_logistic_fit,
                           ncol = 2, nrow = 1)
plot_logistic_fit
ggsave(plot_logistic_fit, filename = paste0("./data/figures/Figure_logistic_fit_",format(Sys.time(), "%Y-%m-%d"), ".png"), height = 4, width = 8,  bg = "transparent")
